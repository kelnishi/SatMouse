/**
 * Windows tray wrapper — the entry point for the packaged Windows app.
 *
 * Creates a Win32 notification area (system tray) icon and spawns the
 * SatMouse server as a child process. Same architecture as the macOS
 * tray-wrapper.ts, but uses Win32 API via koffi instead of ObjC.
 *
 * The tray wrapper owns the UI; the child process runs the bridge server.
 * Communication: Node IPC channel (process.send/on('message')).
 */
...
// NOTIFYICONDATAW offsets (x64)
const NID_SIZE = 976;
const NID_cbSize = 0;
const NID_hWnd = 8;
const NID_uID = 16;
const NID_uFlags = 20;
const NID_uCallbackMessage = 24;
const NID_hIcon = 32;
const NID_szTip = 40;
...
function main() {
  ...
  const className = wstr("SatMouseWrapperClass");

  // Resolve paths — execPath is the node.exe next to main.cjs
  const execDir = dirname(process.execPath);
  const mainCjs = join(execDir, "main.cjs");
    ...
    if (msg === WM_COMMAND) {
      ...
      switch (id) {
        ...
        case ID_OPEN_CLIENT:
          ...
          break;
        case ID_REFRESH:
          serverProcess?.send({ type: "rescan" });
          break;
        case ID_QUIT:
  ...
  wcBuf.writeUInt32LE(80, 0);
  wcBuf.writeUInt32LE(0, 4);
  koffi.encode(wcBuf, 8, "void *", wndProcCb);
  wcBuf.writeInt32LE(0, 16);
  wcBuf.writeInt32LE(0, 20);
  koffi.encode(wcBuf, 24, "void *", hInstance);
  koffi.encode(wcBuf, 64, "void *", className);

  const atom = RegisterClassExW(wcBuf);
  if (!atom) {
    console.error("[Tray] RegisterClassExW failed");
    process.exit(1);
  }
  ...
  if (!hwnd) {
    console.error("[Tray] CreateWindowExW failed");
    process.exit(1);
  }

  // Load icon
  let hIcon: any = null;
  const icoPath = join(execDir, "SatMouse.ico");
  ...
  // Spawn the server process with IPC channel
  ...
  const _refs = [wndProcCb, hwnd, hMenu, hIcon, nidBuf, className, wcBuf];
}
